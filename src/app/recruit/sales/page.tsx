// src/app/recruit/sales/page.tsx
import Image from "next/image";
import Link from "next/link";

type SpecRow = { label: string; value: string | string[] };

const specsSales: SpecRow[] = [
  { label: "雇用形態", value: "正社員" },
  {
    label: "勤務地",
    value: ["北海工務店", "〒063ー0061北海道札幌市西区西町北14丁目1-15ホクシンビル3A"],
  },
    ...
    <main className="bg-white text-gray-900 overflow-x-hidden">
      ...
      <section className="relative">
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          {/* テキスト：スマホで後 */}
          <div className="bg-[#12b3ad] text-white px-6 md:px-14 py-12 md:py-20 flex items-center order-2 md:order-1 min-w-0">
      ...
      {/* キャリア block（スマホは 画像→テキスト の順のまま） */}
      <section className="bg-[#12b3ad] text-white overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="px-6 md:px-14 py-12 md:py-20 space-y-10 order-2 min-w-0">
            ...
            <div className="border-2 border-white/80 p-6 sm:p-8 md:p-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 font-semibold min-w-0">
                <ArrowItem text="売上ノルマを達成するのが難しく現場が厳しい" />
                <ArrowItem text="キャリアアップの制度がなく、給料も上がらなくて将来が不安" />
                <ArrowItem text="自分の営業力を発揮する場所がほしい" />
        ...
        <div className="mx-auto max-w-6xl px-6 pb-20">
          <div className="rounded-2xl overflow-hidden border border-neutral-200">
            ...
            {specsSales.map((row) => (
              <div
                key={row.label}
                className="grid md:grid-cols-3 border-b last:border-b-0 border-neutral-200"
              >
